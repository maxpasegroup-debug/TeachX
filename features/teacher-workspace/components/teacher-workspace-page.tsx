...
function Library({ data, lessonsOnly = false }: { data: WorkspaceData; lessonsOnly?: boolean }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!visible.length ? <EmptyState icon={<BookOpen className="h-5 w-5" />} title={`No ${lessonsOnly ? "lessons" : "resources"} found`} description="Create your first item or adjust the current filters." /> :
...
function Classrooms({ data }: { data: WorkspaceData }) {
  const [query, setQuery] = useState("");
  const items = data.classrooms.filter((item) => `${item.title} ${item.course} ${item.section} ${item.subjects.join(" ")}`.toLowerCase().includes(query.toLowerCase()));
  return <div className="space-y-6"><Input className="max-w-xl" onChange={(event) => setQuery(event.target.value)} placeholder="Search classes, sections, or subjects..." type="search" value={query} />{!items.length ? <EmptyState icon={<UsersRound className="h-5 w-5" />} title="No classrooms found" description="Assigned classrooms will appear here." /> : <div className="grid gap-5 xl:grid-cols-2">{items.map((item) => <Card className="p-5 shadow-soft" key={item.id}><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-semibold">{item.title}</h2><p className="mt-1 text-sm text-muted-foreground">{item.course} - Section {item.section}</p></div><Badge>{item.studentCount} students</Badge></div><div className="mt-5 grid grid-cols-3 gap-3"><div className="rounded-xl bg-background p-3"><p className="text-xs text-muted-foreground">Attendance</p><p className="mt-1 font-semibold">{item.attendanceRate === null ? "No sessions" : `${item.attendanceRate}%`}</p></div><div className="rounded-xl bg-background p-3"><p className="text-xs text-muted-foreground">Resources</p><p className="mt-1 font-semibold">{item.resourceCount}</p></div><div className="rounded-xl bg-background p-3"><p className="text-xs text-muted-foreground">Review</p><p className="mt-1 font-semibold">{item.homeworkPending}</p></div></div><p className="mt-4 text-sm"><strong>Subjects:</strong> {item.subjects.join(", ") || "Not assigned"}</p><details className="mt-4 rounded-xl border border-border p-3"><summary className="cursor-pointer text-sm font-semibold">Timetable</summary><div className="mt-3 space-y-2">{item.timetable.map((entry) => <div className="flex justify-between gap-3 rounded-lg bg-background p-3 text-sm" key={entry.id}><span>{entry.day} - {entry.subject}</span><span className="text-muted-foreground">{entry.time} - {entry.room}</span></div>)}</div></details><details className="mt-3 rounded-xl border border-border p-3"><summary className="cursor-pointer text-sm font-semibold">Classroom notes</summary><div className="mt-3 space-y-2">{item.notes.length ? item.notes.map((note) => <p className="rounded-lg bg-background p-3 text-sm" key={note}>{note}</p>) : <p className="text-sm text-muted-foreground">No classroom notes yet.</p>}</div></details><div className="mt-4 flex flex-wrap gap-2"><Link className="rounded-full bg-primary px-4 py-2 text-xs font-medium text-primary-foreground" href={`/classrooms/${item.id}`}>Open classroom</Link><Link className="rounded-full border border-border px-4 py-2 text-xs font-medium" href={`/classrooms/${item.id}#attendance`}>Attendance</Link><Link className="rounded-full border border-border px-4 py-2 text-xs font-medium" href={`/classrooms/${item.id}#assignments`}>Homework</Link><Link className="rounded-full border border-border px-4 py-2 text-xs font-medium" href={`/classrooms/${item.id}#materials`}>Resources</Link></div></Card>)}</div>}</div>;
}
