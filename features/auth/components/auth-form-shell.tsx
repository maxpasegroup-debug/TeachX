  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-surface px-5 py-8 text-foreground sm:px-8">
      ...
      <header className="mx-auto flex w-full max-w-7xl items-center justify-between">
        ...
        <Link className="nav-link rounded-full px-4 py-2 text-sm font-semibold text-muted-foreground transition duration-brand ease-brand hover:bg-muted hover:text-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2" href="/">
          Back Home
        </Link>
      </header>
      ...
      <section className="mx-auto grid min-h-[calc(100vh-7rem)] w-full max-w-7xl items-center gap-10 py-10 lg:grid-cols-[0.95fr_0.8fr]">
        <MotionPrimitive className="order-2 lg:order-1" variant="fade-right">
          <div className="premium-glass-card relative overflow-hidden rounded-[2.25rem] border border-white/75 bg-white/90 p-7 shadow-brand sm:p-10">
            ...
            <div className="relative">
              ...
              <div className="mt-10 grid gap-3 sm:grid-cols-2">
                {trustItems.map((item) => {
                  ...
                  return (
                    <div className="premium-soft-tile rounded-2xl border border-white/80 bg-white/90 px-4 py-4 text-sm font-semibold text-muted-foreground shadow-sm" key={item.label}>
        ...
        <MotionPrimitive className="order-1 lg:order-2" variant="scale">
          <section aria-labelledby="auth-title" className="premium-glass-card rounded-[2rem] border border-white/80 bg-white/95 p-6 shadow-brand sm:p-8">
