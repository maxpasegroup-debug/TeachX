...
import { ArrowRight, Bell, BookOpen, Bookmark, Brain, CalendarDays, CheckCircle2, CircleDollarSign, Clock3, Download, FileText, FolderOpen, GraduationCap, Heart, History, Lightbulb, NotebookPen, Pin, Sparkles, Target, UsersRound } from "lucide-react";
...
export function TeacherOperatingDashboard({ name, completion, recentItems, favorites, savedDrafts, notifications, plan, aiCreditsRemaining, stats, daily, canAccessInstitution }: DashboardProps & {
  ...
  return (
    <div className="space-y-7">
      <section className="relative min-h-[360px] overflow-hidden rounded-md border bg-surface shadow-soft">
        <Image alt="A teacher planning calmly in a bright classroom" className="object-cover object-center lg:object-right" fill priority sizes="(max-width: 1024px) 100vw, 82vw" src="/teacher-life-os-home.webp" />
        <div className="absolute inset-y-0 left-0 w-full bg-white/90 lg:w-[58%]" aria-hidden="true" />
        <div className="relative z-10 flex min-h-[360px] max-w-2xl flex-col justify-center p-5 sm:p-8 lg:p-10">
          <Badge>Teacher Life OS</Badge><p className="mt-5 text-sm font-semibold text-sky-800">{today}</p><h1 className="mt-2 text-3xl font-semibold sm:text-4xl">Good morning, {firstName(name)}</h1><p className="mt-3 text-lg font-medium">Teach better. Work smarter. Grow. Live better.</p><p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">Choose what you want to achieve. TeachX will take you to the right tools, with TARA beside you.</p>
          <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-3 text-sm"><span><strong>{plan}</strong> plan</span><span><strong>{aiCreditsRemaining}</strong> AI credits available</span><Link className="inline-flex min-h-11 items-center gap-2 rounded-md bg-sky-700 px-4 font-semibold text-white focus:outline-none focus:ring-2 focus:ring-sky-700 focus:ring-offset-2" href="/tara">Ask TARA<ArrowRight className="h-4 w-4" /></Link></div>
        </div>
      </section>
      <section><div className="mb-4"><h2 className="text-2xl font-semibold">What would you like to do?</h2><p className="mt-1 text-sm text-muted-foreground">Four clear ways to use your Teacher Life OS.</p></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
        { title: "Save Time", detail: "Your AI-powered teaching workspace.", actions: "Plan · Create · Teach · Organize", href: "/teacher/life/save-time", icon: Clock3, tone: "border-sky-200 bg-sky-50 text-sky-900" },
        { title: "Earn More", detail: "Turn your knowledge into opportunity.", actions: "Teach 1:1 · Publish · Build Profile", href: "/teacher/life/earn-more", icon: CircleDollarSign, tone: "border-emerald-200 bg-emerald-50 text-emerald-900" },
        { title: "Learn More", detail: "Invest in yourself.", actions: "AI Skills · Listen · Learn · Join", href: "/teacher/life/learn-more", icon: Brain, tone: "border-amber-200 bg-amber-50 text-amber-950" },
        { title: "Enjoy More", detail: "A better life beyond the classroom.", actions: "Coming Soon", href: "/teacher/life/enjoy-more", icon: Heart, tone: "border-rose-200 bg-rose-50 text-rose-950" }
      ].map(({ title, detail, actions, href, icon: Icon, tone }) => <Link className={`group min-w-0 rounded-md border p-5 shadow-soft transition hover:-translate-y-1 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary motion-reduce:transform-none ${tone}`} href={href} key={title}><span className="grid h-11 w-11 place-items-center rounded-md bg-white/80"><Icon className="h-5 w-5" /></span><h3 className="mt-5 text-xl font-semibold">{title}</h3><p className="mt-2 min-h-10 text-sm">{detail}</p><p className="mt-4 text-xs font-semibold leading-5 opacity-75">{actions}</p><span className="mt-4 inline-flex items-center text-sm font-semibold">Explore <ArrowRight className="ml-2 h-4 w-4 transition group-hover:translate-x-0.5 motion-reduce:transform-none" /></span></Link>)}</div></section>
