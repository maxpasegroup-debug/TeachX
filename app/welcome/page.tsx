...
const paths = [
  {
    ...
    description: "Create AI-assisted lessons, organize resources, build your professional profile, and grow your teaching business.",
    href: "/signup/teacher",
    icon: Sparkles
  }
];
      ...
      <section className="w-full max-w-5xl">
        <Link className="mb-10 inline-flex items-center gap-3" href="/">
          ...
          <span>
            ...
            <span className="block text-xs text-muted-foreground">Professional AI Workspace</span>
          </span>
        </Link>
        <div className="mb-10 max-w-3xl">
          <p className="mb-3 text-sm font-medium text-sky-700">Welcome to TeachX Guru</p>
          <h1 className="text-4xl font-semibold tracking-tight sm:text-6xl">Enter your professional teaching workspace</h1>
          <p className="mt-5 text-lg leading-8 text-muted-foreground">TeachX Guru is built exclusively for teachers who want AI creation, resource management, marketplace presence, and teaching growth in one calm workspace.</p>
        </div>
          ...
          {paths.map((path) => {
            ...
            return (
              <Link className="group rounded-[1.75rem] border border-border bg-surface p-7 shadow-soft transition hover:-translate-y-1 hover:border-sky-200" href={path.href} key={path.title}>
                ...
                <p className="mt-3 max-w-2xl leading-7 text-muted-foreground">{path.description}</p>
