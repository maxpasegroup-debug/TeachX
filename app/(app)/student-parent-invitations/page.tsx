...
export default async function ParentInvitationsPage() {
  ...
  const parentId = user?.roles.includes("PARENT") ? user.id : null;
  const records = parentId ? await prisma.userPreference.findMany({ where: { key: "learnx.parent-invitations" }, include: { user: { select: { id: true, name: true, email: true } } } }) : [];
  const invitations = records.flatMap((record) => (Array.isArray(record.value) ? record.value as Array<Record<string, unknown>> : []).filter((item) => item.parentId === parentId).map((item) => ({ id: String(item.id), relation: String(item.relation ?? "Parent"), student: record.user })));
